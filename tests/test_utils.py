...
@pytest.mark.parametrize("git_cmd_runner", [utils.GitCurrentTagCommandRunner(), utils.GitLatestTagCommandRunner()])
def test_get_tag(monkeypatch, cli_stdout, expected, git_cmd_runner):
    ...
    assert git_cmd_runner.run() == expected
...
def restore_toml_fixture(toml_path: pathlib.Path) -> Generator:
    with open(toml_path, encoding="utf8") as file:
...
def test_get_current_tag_raises_error_if_there_is_no_tag(monkeypatch):
    ...
    with pytest.raises(RuntimeError):
        utils.GitCurrentTagCommandRunner().run()
